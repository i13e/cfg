...
local on_attach = function(client, bufnr)
	...
	map("n", "K", "<cmd>lua vim.lsp.buf.hover()<CR>", { desc = "hover information [LSP]" })
	...
	map("n", "gm", "<cmd>lua vim.lsp.buf.implementation()<CR>", { desc = "goto implementation [LSP]" })
	map("n", "gy", "<cmd>lua vim.lsp.buf.type_definition()<CR>", { desc = "goto type definition [LSP]" })
	map("n", "gA", "<cmd>lua vim.lsp.buf.code_action()<CR>", { desc = "code actions [LSP]" })
	map("v", "gA", "<cmd>lua vim.lsp.buf.range_code_action()<CR>", { desc = "range code actions [LSP]" })
	...
	map("n", "<leader>K", "<cmd>lua vim.lsp.buf.signature_help()<CR>", { desc = "signature help [LSP]" })
	map("n", "<leader>k", '<cmd>lua require("lsp.handlers").peek_definition()<CR>', { desc = "peek definition [LSP]" })
	...
	map(
		"n",
		"[d",
		("<cmd>lua vim.diagnostic.goto_prev(%s)<CR>"):format(winopts),
		{ desc = "previous diagnostic [LSP]" }
	)
	map("n", "]d", ("<cmd>lua vim.diagnostic.goto_next(%s)<CR>"):format(winopts), { desc = "next diagnostic [LSP]" })
	map("n", "<leader>lc", "<cmd>lua vim.diagnostic.reset()<CR>", { desc = "clear diagnostics [LSP]" })
	map(
		"n",
		"<leader>ll",
	...
	map("n", "<leader>lq", "<cmd>lua vim.diagnostic.setqflist()<CR>", { desc = "send diagnostics to quickfix [LSP]" })
	map("n", "<leader>lQ", "<cmd>lua vim.diagnostic.setloclist()<CR>", { desc = "send diagnostics to loclist [LSP]" })

	if client_has_capability(client, "documentFormattingProvider") then
		map("n", "gq", "<cmd>lua vim.lsp.buf.formatting()<CR>", { desc = "format document [LSP]" })
	end
	if client_has_capability(client, "documentRangeFormattingProvider") then
		map("v", "gq", "<cmd>lua vim.lsp.buf.range_formatting()<CR>", { desc = "format selection [LSP]" })
	end

	if client_has_capability(client, "codeLensProvider") then
		map("n", "<leader>lL", "<cmd>lua vim.lsp.codelens.run()<CR>", { desc = "[LSP] code lens" })
