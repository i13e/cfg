...
function M.preview_location(loc, _, _)
	...
	local pos = range.start.line == range["end"].line
			and { before + 1, range.start.character + 1, range["end"].character - range.start.character }
		or { before + 1 }
...
function M.peek_definition()
	...
	if vim.tbl_contains(vim.api.nvim_list_wins(), _float_win) then
		...
		return vim.lsp.buf_request(0, "textDocument/definition", params, M.mk_handler(M.preview_location_callback))
	end
end
